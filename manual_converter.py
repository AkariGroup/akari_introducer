    ...
    for i, line in enumerate(lines):
        ...
        if line.strip() in {
            ...
            if i > 0 and lines[i - 1].strip():
                result.append(f"file://{convert_path_to_public_url(path)}#{lines[i - 1]}")
            else:
...
def replace_decorative_lines_for_public_url(path: str, text: str):
    ...
    for i, line in enumerate(lines):
        ...
        if line.strip() in {
            ...
            # 一つ前の行が存在し、かつ空行でない場合のみ置き換える
            if i > 0 and lines[i - 1].strip():
                result.append(f"{AKARI_DOC_URL}{convert_path_to_public_url(path)}#{lines[i - 1]}")
            else:
                result.append(line)  # 置き換え条件を満たさない場合はそのまま追加
        else:
            result.append(line)
...
def main() -> None:
    ...
    for file_path in file_paths:
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            text = file.read()
            # 関数を呼び出して結果を出力
            output_text = replace_decorative_lines_for_public_url(path=file_path, text=text)
            if args.save_path:
                path = f"{args.save_path}/{os.path.basename(file_path)}"
